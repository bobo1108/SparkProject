...
object RealTimeHotSong {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //转换成需要的类型
    var fromOffsets: Predef.Map[TopicPartition, Long] = currentTopicOffset.map { resultSet =>
      new TopicPartition(userPlaySongTopic, resultSet._1.toInt) -> resultSet._2.toLong
    }.toMap

    val pros = new Properties()
    pros.put("bootstrap.servers",kafkaBrokers)
    pros.put("key.deserializer",classOf[StringDeserializer])
    pros.put("value.deserializer",classOf[StringDeserializer])
    pros.put("group.id","group.id")
    ...
    /**
      ...
      */
    val topics = List(userPlaySongTopic)
